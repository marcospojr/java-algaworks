package com.marcosjr.fjoo.collections;
...
public class Governo {
	...
	public BigDecimal calcularGastosComSalario(String partidoPolitico) {
		BigDecimal totalGastoSalario = BigDecimal.ZERO;
		List<Politico> politicos = this.partidosPoliticos.get(partidoPolitico);
		
		if(politicos != null) {
			for(Politico politico : politicos) {
				totalGastoSalario = totalGastoSalario.add(politico.getCargo().getSalario());
			}
		}
		
		return totalGastoSalario;
	}
	
	public BigDecimal calcularGastosComSalarioParaCargo(Cargo cargo, String partidoPolitico) {
		BigDecimal totalSalarioCargo = BigDecimal.ZERO;
		List<Politico> politicos = this.partidosPoliticos.get(partidoPolitico);
		
		if(politicos != null) {
			for(Politico politico : politicos) {
				if (politico.getCargo().equals(cargo)) {
					totalSalarioCargo = totalSalarioCargo.add(politico.getCargo().getSalario());
				}
			}
		}
		
		return totalSalarioCargo;
	}
